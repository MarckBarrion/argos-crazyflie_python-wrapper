...
#include <argos3/plugins/robots/crazyflie/simulator/qtopengl_crazyflie.h>
...
CPyQTUserFunction::CPyQTUserFunction() {
  ...
  RegisterUserFunction<CPyQTUserFunction,CCrazyflieEntity>(&CPyQTUserFunction::Draw);
}
...
void CPyQTUserFunction::Draw(CCrazyflieEntity& c_entity) {
    /* The position of the drawings is expressed wrt the reference point of the Crazyflie */

    // // Create an instance of the OpenGL Crazyflie model
    // static argos::CQTOpenGLCrazyflie m_cQTOpenGLCrazyflie;

    // // Draw the Crazyflie entity using the OpenGL model
    // m_cQTOpenGLCrazyflie.Draw(c_entity);

    // Get the Crazyflie controller and export to Python
    ...
    // Launch Python draw function
    try {
        object draw_in_robot_f = m_qtuser_main.attr("draw_in_robot");
        draw_in_robot_f();
    } catch (error_already_set) {
        PyErr_Print();
    }
